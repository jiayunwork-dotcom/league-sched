// Package standings validates match results and computes league tables.
package standings
...
// Result is a played match.
type Result struct {
	Home, Away           string
	HomeGoals, AwayGoals int
}

// Row is one team's line in the table.
type Row struct {
	Team   string
	Played int
	Won    int
...
// PointsPerWin / PointsPerDraw define the scoring system.
const (
	PointsPerWin  = 3
	PointsPerDraw = 1
)

// Table computes the standings of teams after results. Rows are sorted by
// points, then goal difference, then goals for, then team name. Unknown
// teams or negative goals are rejected.
